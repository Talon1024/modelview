...
matrix PCS_Model::moi_recalculate(float X, float Z, float xw, float zw){
	std::vector<vector3d> cpoints;
	if(!moi_colide(cpoints, X, Z))
		return matrix(vector3d(), vector3d(), vector3d());//return null
	//moi colide ensures there is an even number of colisions and orders them into pairs
	matrix ret = moi_recalculate(X,Z,xw,zw,cpoints[0].y,cpoints[1].y-cpoints[0].y);
...
matrix PCS_Model::moi_recalculate(float X, float Z, float xw, float zw, float Y, float dy){
	matrix ret;
