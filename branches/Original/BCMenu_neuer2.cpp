...
BOOL BCMenu::original_select_disabled=FALSE;
...
BCMenu::BCMenu()
{
	...
	//HH-Code
	m_XPMode=1-(AfxGetApp()->GetProfileInt("Settings","DisableXPMenuStyle",0));
	original_drawmode=m_XPMode;
	xp_drawmode=m_XPMode;
	if(m_XPMode==0)
	{
		m_iconX=16;
		m_iconY=15;
...
int BCMenu::GetMenuDrawMode(void)
{
	...
	//return 1-AfxGetApp()->GetProfileInt("Settings","DisableXPMenuStyle",0);
}
...
void BCMenu::MeasureItem( LPMEASUREITEMSTRUCT lpMIS )
{
	...
	if(state & MF_SEPARATOR){
		...
		if(m_XPMode==0)
			...
			lpMIS->itemHeight=3; //HHCHANGE GetSystemMetrics(SM_CYMENU)>>1;
	}
	else{
		CFont m_fontMenu;
		LOGFONT m_lf;
