...
TRACE("Deleting texture %i from memory\n",j);
	...
	m_D3_Model.PINF[0]=0;
	for(unsigned long i=0;i<MAX_D3_SOBJ;i++)
	{
		m_D3_HiLiteSobj[i]=FALSE;
...
void CMODVIEW32Doc::Serialize(CArchive& ar)
{
	...
	if((an.Find(".HAM")>=0) || (an.Find(".HXM")>=0))
	{
		...
		else //Supported are DESCENT2.HAM, V-HAM and HXM formats
		{
			...
			strcpy(m_D2_CurrHAXM_Filename,ar.GetFile()->GetFilePath());
			m_ErrorString=ERROR_GEN_NOMODELSELECTED_TEXT;
			m_ErrorCode=ERROR_GEN_NOMODELSELECTED;
		}
	}
...
void CMODVIEW32Doc::ExplorerAddChild(char *filename, int ModelNum/*=-1*/)
{
	...
	if(m_Game==GAME_D2)
	{
		//Descent 2 is handled internally
		typenr=m_D2_PolygonModels[ModelNum].Type;
	}
	else
	{
		char type[256];
...
ERRORCODE CMODVIEW32Doc::FS_LoadPCXData(unsigned long ActivePM,BOOL bAniLoad,BOOL bFastload)
{
	...
	if(m_FS_BitmapData.count>0)
	{
		for(i=m_FS_PofDataL[ActivePM];i<(m_FS_PofDataH[ActivePM]+1);i++)
		{
			...
			for(int j=0;j<m_FS_Model.Pcount;j++)
			{	// type 3 polygons
			...
			strcpy(Filename,m_FS_BitmapData.pic[i].name);
			strcat(Filename,".pcx");
...
int cfseek(CFile *fp,long offset,int whence)
{
//  printf("%#08lx\n",ftell(fp));
...
ERRORCODE CMODVIEW32Doc::FS_ReadPOF(CFile *fp,int VpNum)
{
	...
	unsigned long len,next_chunk;
	unsigned long temp,i,j,pos_start; 
	...
	TRACE("%i <%c%c%c%c> %i %d\n",pos_start,*pid,*(pid+1),*(pid+2),*(pid+3),len,m_FS_NumSOBJ);
		...
		case ID_HDR2:
			{		//Object header
				...
				for(i=0;i<m_FS_PMinfo.n_models;i++)
				{
				...
				if(m_FileVersion>=1903)
				{
					...
					if(m_FileVersion<2009)
					{
						...
						float vol_mass=m_FS_ModelInfo.mass;
						float areamass=4.65*pow(m_FS_ModelInfo.mass,2/3);
					...
					for(int j1=0;j1<3;j1++)
					{
						for(int j2=0;j2<3;j2++)
							m_FS_ModelInfo.moment_inertia[j2][j1]=read_float(fp)*scale;
					}
				} else {
		...
		case ID_TXTR:
			{		//Texture filename list
				...
				for(i=0;i<m_FS_BitmapData.count;i++)
				{
					...
					strcpy(m_FS_BitmapData.pic[i].name,inputbuf);
				}
				break;
			}
		...
		case ID_DOCK:
			{
				...
				for(int i=0;i<m_FS_NumDocks;i++)
				{
					...
					for(j=0;j<m_FS_Docks[i].num_points;j++)
					{
...
void CMODVIEW32Doc::FS_PreparePOF()
{
	...
	for(i=0;i<m_FS_NumSOBJ;i++)
		m_FS_SOBJ[i].detail=-1;
	for(i=0;i<m_FS_PMinfo.n_detail;i++)
		m_FS_SOBJ[m_FS_PMinfo.detail[i]].detail=i;
	for(i=0;i<m_FS_PMinfo.n_debris;i++)
	...
	int detaillevel=0; 
	for(i=0;i<m_FS_Model.Vcount;i++)
	{
		if(m_FS_SOBJ[m_FS_Model.VSobj[i]].detail==(long)detaillevel)
			m_NumVertices++;
	}
	for(i=0;i<m_FS_Model.Pcount;i++)
	{
		if(m_FS_SOBJ[m_FS_Model.Poly[i].Sobj].detail==(long)detaillevel)
			m_NumPolygons++;
	}
	for(i=m_FS_PofDataL[0];i<(m_FS_PofDataH[0]+1);i++)
	{
	...
	for(i=0;i<m_FS_NumSOBJ;i++)
	{
		if(m_FS_SOBJ[i].detail==(long)detaillevel)
			m_NumSubmodels++;
	}
...
{
	unsigned long id,len,next_chunk,btemp;
	DWORD ooflength;
	unsigned long i,j;
	unsigned long version;
	float ftemp;
	...
	while(ooflength>fp->GetPosition())
	{
		...
		m_Blocks_Num++;
		m_Blocks[m_Blocks_Num-1].name.Format("%c%c%c%c",*pid,*(pid+1),*(pid+2),*(pid+3));
		m_Blocks[m_Blocks_Num-1].offset=fp->GetPosition()-OffsetStart;
		m_Blocks[m_Blocks_Num-1].size=len;
		m_Blocks[m_Blocks_Num-1].desc="??? <Unknown>";
		...
		case ID_OHDR: {		//Object header
				...
				m_D3_Model._minbox=(D3_VPNT)read_VECTOR(fp); //pmmin
...
TRACE("%s - %s\n",m_RF_Name,m_RF_None);
	...
	for(i=0;i<m_RF_Model.num_geounits;i++)
	{
...
//TRACE("numtextures=%i\n",m_RF_Model.num_textures);
	...
	for( i=0 ; i<m_RF_Model.num_textures; i++ )
	{
...
BOOL CMODVIEW32Doc::EditorFS_SavePOF(CString filename, int fileversion)
{
	...
	for(guntype=0;guntype<2;guntype++)
	{
...
unsigned char * CMODVIEW32Doc::EditorFS_GenerateTextures()
{
	...
	for(int i=0;i<m_FS_BitmapData.count;i++)
	{
...
unsigned char * CMODVIEW32Doc::EditorFS_GenerateModelHeader()
{
	...
	for(int i=0;i<m_FS_PMinfo.n_detail;i++)
	...
	for(i=0;i<m_FS_ModelInfo.num_cross_sections;i++)
	...
	for(i=0;i<m_FS_ModelInfo.num_lights;i++)
...
unsigned char * CMODVIEW32Doc::EditorFS_GenerateShields()
{
	...
	for(int vert=0;vert<m_FS_Model.shields.Vcount;vert++)
	...
	for(int face=0;face<m_FS_Model.shields.Fcount;face++)
