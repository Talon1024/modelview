...
class PCS_Model 
{
	private:
		...
		matrix moi_recalculate(float X, float Z, float xw, float zw);
		matrix moi_recalculate(float X, float Z, float xw, float zw, float Y, float dy);
		bool moi_colide(std::vector<vector3d>&cpoints, float x, float z);

	public:
		matrix moi_recalculate(int Xres, int Yres);
